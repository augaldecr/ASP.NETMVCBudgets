...
using ASP.NETMVCBudgets.Servicios;
using AutoMapper;
...
namespace ASP.NETMVCBudgets.Controllers
{
    public class TransaccionesController : Controller
    {
        ...
        public async Task<IActionResult> Index(int mes, int anho)
        {
            var usuarioId = servicioUsuarios.ObtenerUsuarioId();
            var modelo = await servicioReportes
                .ObtenerReporteTransaccionesDetalladas(usuarioId, mes, anho, ViewBag);
            return View(modelo);
        }

        public async Task<IActionResult> Semanal(int mes, int anho)
        {
            var usuarioId = servicioUsuarios.ObtenerUsuarioId();
            IEnumerable<ResultadoObtenerPorSemana> transaccionesPorSemana =
                await servicioReportes.ObtenerReporteSemanal(usuarioId, mes, anho, ViewBag);
            ...
            var fechaReferencia = new DateTime(anho, mes, 1);
            ...
            {
                var semana = i + 1;
                var fechaInicio = new DateTime(anho, mes, diasSegmentados[i].First());
                var fechaFin = new DateTime(anho, mes, diasSegmentados[i].Last());
        ...
        public async Task<IActionResult> Mensual(int anho)
        {
            var usuarioId = servicioUsuarios.ObtenerUsuarioId();

            if (anho == 0)
            {
                anho = DateTime.Today.Year;
            }

            var transaccionesPorMes = await repositorioTransacciones.ObtenerPorMes(usuarioId, anho);
        ...
        [HttpGet]
        public async Task<FileResult> ExportarExcelPorMes(int mes, int anho)
        {
            var fechaInicio = new DateTime(anho, mes, 1);
        ...
        [HttpGet]
        public async Task<FileResult> ExportarExcelPoranho(int anho)
        {
            var fechaInicio = new DateTime(anho, 1, 1);
